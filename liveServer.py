...
def generate_from_preview_route():
    ...
    try:
        ...
        # Create a path for a temporary HTML file to pass to Puppeteer
        temp_html_path = os.path.join(temp_dir, 'input_for_puppeteer.html')

        # Write the HTML content to this file
        with open(temp_html_path, 'w', encoding='utf-8') as f:
            f.write(html_content)

        # Update the arguments to pass the file PATH instead of the raw content
        puppeteer_args = {
            'htmlPath': temp_html_path,      # <-- FIXED: Pass the path, not the content
