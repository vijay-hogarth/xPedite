...
# --- Robust Pathing Helper Function for PyInstaller ---
...
# --- Path Configuration (Updated for Robustness) ---
PUPPETEER_SCRIPT_DIR = resource_path('puppeteer_pdf_generator')
PUPPETEER_SCRIPT_PATH = os.path.join(PUPPETEER_SCRIPT_DIR, 'dummyGen.js')

# Also update your other folder paths for consistency
UPLOAD_FOLDER = resource_path('uploads')
OUTPUT_FOLDER = resource_path('generated_pdfs')
CONFIG_FILE = resource_path('pdf_configs.json')
...
def delete_generated_file(filename):
    ...
    return redirect(url_for('list_generated_files'))
...
def upload_files_and_generate_pdf():
    ...
    temp_dir_for_upload = tempfile.mkdtemp()
    try:
        for file in files:
...
def preview_config():
    ...
    try:
        ...
        return jsonify({
            ...
            'estimated_pages': max(1, len(preview_data['sections']) * 2)
        })
    except Exception as e:
        return jsonify({
            'success': False,
            'error': f'Error generating preview: {str(e)}',
            'preview': {'name': '', 'title': '', 'description': '', 'sections': []}
        }), 500
...
def generate_from_preview_route():
    ...
    try:
        ...
        output_path = os.path.join(OUTPUT_FOLDER, output_filename)
        temp_html_path = os.path.join(temp_dir, 'input_for_puppeteer.html')
        with open(temp_html_path, 'w', encoding='utf-8') as f:
            f.write(html_content)
        puppeteer_args = {
            'htmlPath': temp_html_path,
...
def extract_pages_from_pdf(pdf_path, output_dir, prefix="", extract_mode="all", page_numbers=None, dpi=150):
    ...
    if not PYMUPDF_AVAILABLE:
        error_msg = "Critical Error: PyMuPDF (fitz) is not installed. It must be in requirements.txt"
        print(f"❌ {error_msg}")
    ...
    try:
        ...
        if extract_mode == "single" and page_numbers:
            pages_to_extract = [page_numbers[0]]
...
def exported_images_gallery():
    """
    Displays a gallery of previously exported images.
    """
    ...
    try:
        ...
        if pdf_quality == 'high':
            ...
            try:
                ...
                pdf_dpi = max(50, min(600, pdf_dpi))
